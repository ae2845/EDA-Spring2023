...
# 1. Use `here` with `read.csv() to read in a raw dataset into a dataframe.
#library(here)
#library(tidyverse)
raw_data <- here('Data','Raw','NTL-LTER_Lake_ChemistryPhysics_Raw.csv')
print(raw_data)
...
# 2. Read in all CSV files that start with "EPAair_O3" into a single dataframe.
# Creating a file path.
EPAair_O3_Folder <- here('Data', 'Raw')
EPAair_O3_Folder

# Selecting only files with "EPAair_O3" in them, and putting them into a list.
EPAair_O3_Files <- dir(EPAair_O3_Folder, pattern = "EPAair_O3")
EPAair_O3_Files

# I tried many variations on the code below, suggested by Jenny Richmond on her website, but I could not get it to work. I would continually get an error that the 2018 file did not exist in the working directory.
EPAair_O3_Data <- (EPAair_O3_Files) %>%
  map(read_csv) %>%
  reduce(rbind)

# Eventually I was able to get this to work, after looking at a separate webpage from the above author, with the following modification: "map(~ read_csv(file.path(EPAair_O3_Folder, .)))" I think this modification was needed because the files lived in a subfolder of my working directory.
EPAair_O3_Data <- (EPAair_O3_Files) %>%
  map(~ read_csv(file.path(EPAair_O3_Folder, .))) %>%
  reduce(rbind)
EPAair_O3_Data
