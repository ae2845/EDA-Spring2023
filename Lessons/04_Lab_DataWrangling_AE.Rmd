...
> Add notes here: 
These long-term observations would help to tell us about quality measures for the lake, which would also apply to lake habitat and aquatic life within the ecosystem. 
...
# Class of year4 is integer:

NTL.phys.data.1999.ex1 <- filter(NTL.phys.data, year4 == 1999)
NTL.phys.data.1999.ex2 <- filter(NTL.phys.data, year4 %in% c(1999))
...
# Due to the leap year issue (in 1992 and 2004, 182 was represented as June, not July 1st) it made the most sense to creae a new column of the month using mutate, as the instructors hinted.[NTL.phys.data.july1 <- filter(NTL.phys.data, daynum == 182)]. 

NTL.phys.data.TL.1990.1999 <-
  NTL.phys.data %>%
...
NTL.phys.data.TL.summary <-
  NTL.phys.data.TL.1990.1999 %>%
  filter(depth == 0, !is.na(temperature_C)) %>%
...
NTL.phys.data.irradiance.gathered <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")

#Exercise 6: Spread temperatureC into more than one column based on the depth.

NTL.phys.data.depth.temperature.spread <-
  NTL.phys.data %>%
  select(sampledate, temperature_C, depth) %>%
