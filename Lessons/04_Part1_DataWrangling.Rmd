...
> Add notes here: 
These observations would help to tell us about lake quality and hence quality
for aquatic life. 
...
#Remember that if you are comparing characters (or factors?) then you must 
#put the object in quotes to do the comparison, unlike numbers.

# matrix (notation) filtering (since dataframes work as a matrix, can filter
# them this way -- LML). In this case, we're only filtering the rows (x) [x,y] #that have a depth of 0.
...
NTL.phys.data.surface4 <- filter(NTL.phys.data, depth < 0.50)
...
# Choose multiple conditions to filter. Depending on how many 
# items you want to filter, you could decide to filter by == or !=
...
# This method using %in% will be useful when you need to select more than a couple items. 
# So you won't have to keep repeating "lakename ==..." It's a bit cleaner this way.

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
# Would look at data in Excel to see where daynum matched up with our date range.
# 152 is June 1st; 304 is Oct 31st.
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305)
# Can use a comma instead of `&` to represent a combination of both requests.
...
# Here again we can use the %in% operator, which we can see as a value-matching 
# operator. So it will match the value of whatever is put inside the vector c(a:b).
...
NTL.phys.data.1999.1 <- filter(NTL.phys.data, year4 == 1999)
NTL.phys.data.1999.2 <- filter(NTL.phys.data, year4 %in% c(1999))

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.TL1 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 >= 1990 & year4 <= 1999)
NTL.phys.data.TL2 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
...
> Answer: 
I'm not sure if I understand this question, but I'm guessing we don't want to filter using row numbers because the row number doesn't provide any meaningful information, unless we know specifically how the data is ordered. We filter based on column variables, thus selecting the rows with the column values we're looking for.
...
# Dates from 6 highest temps: 1995-06-19 (x2), 1998-07-16, 2002-07-01 (x2) and 2002-07-19 	
# Lakes from 6 highest temps: East Long Lake (x3) and Hummingbird Lake (x3)
# Depths from 6 highest temps: 0.0 (x3) and 0.5 (x3)
...
# Note that the item on the left of the = sign is the name of the new column you created.
```
...
```{r}
# add a month column to the dataset; month() extracts just month from a date object.
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) 

# reorder columns to put month with the rest of the date variables; select can be used 
# to pull out # columns we want into a new frame, but can also be used to change the 
# order of the columns as below.
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)

# find out the start and end dates of the dataset. The *interval* function in lubridate
# creates an interval object with a specified start and end date.

# In this example (let's just say we know our data is ordered by date) we can look at 
# the interval between the first observation [1] and the last one [21613].
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613])

# But what if your dataset is NOT ordered by date -- is there another way to calculate the 
# interval? YES -- using function `first` and function `last` from the dplyr package. 
# This is a great approach that will work for any size dataset.
...
#I'm a little confused about this -- this isn't the file we just processed. Wouldn't it be the following dataset instead?

write.csv(NTL.phys.data.processed, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_ChemistryPhysics_PeterPaul2_Processed.csv")
...
>Answer
We do this to keep the R script clean and tidy for reproducibility and sharing.
