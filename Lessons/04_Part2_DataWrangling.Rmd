...
# Notice that the other lake names didn't go away, even though they have zero 
# values. You can drop these lake names completely by using `droplevels` This
# function will remove the bits that were filtered out from previous function.
...
NTL.nutrient.data.PeterPaul <- # result of pipe stored in original dataframe
  NTL.nutrient.data.PeterPaul %>% # selecting the dataframe
  mutate(month = month(sampledate)) %>% # creating month column from sampledate
  select(lakeid:daynum, month, sampledate:comments) %>% # Reordering columns
  drop_na(depth) # dropping all NA values in depth
...
*Note that these functions are similar to the pivot table function in Excel*

*pivot_longer* (older version is gather) is transforming columns into rows,
so you're making the data longer.

*pivot_wider* (older version is spread) is transforming rows into columns,
so you're making the data wider.


```{r}
# Gather nutrient data into one column, old style with `gather`
#NTL.nutrient.data.PeterPaul.gathered <- gather(NTL.nutrient.data.PeterPaul.subset, "nutrient", "concentration", tn_ug:po4)

# Gather nutrient data into one column using pivot_longer. We will turn
# four nutrient-related columns into two nutrient-related columns. 
# One column will have the nutrient name and the 2nd column will have
# the nutrient value for that name = all nutrient values in one column.
# So, 2406 observations became 12030, and thus became "longer."
...
# But our "concentration" column has a lot of NAs. Here we'll use the `subset`
# function which will do something similar to `filter`. is.na will return a 
# logical operator.
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration))
# Count is asking, for each nutrient, how many observations do we have?
# syntax is count(dataset, column)
...
# This is the opposite to what we did above with pivot_longer.
# This dataset now has 2404 obs, but the original had 2406. What probably happened 
# is that a row had NAs for every nutrient and so now is no longer in the set.
...
# Note that by using `separate` in the example below, the "sampledate" column 
# disappears and is replaced by "Y", "m", and "d". 
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate, c("Y", "m", "d"))

# LL recommends using lubridate rather than separate and unite.
...
```{r}
# Notice how you don't have to tell R which columns to join on, it's automatic.
# In the console, it will show you how it did the joining:
# "Joining with `by = join_by(lakename, year4, daynum, month, sampledate, depth)"
# These 6 common columns come first when you look at the joined dataset.
...
```{r}
# This method would be really tedious for 20 separate datasets...
...
```{r}
# The better method is to create a list using the list.files function
# However, I'm not sure why we're using "=" here instead of "<-"
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE)
LitterFiles

# Then use a pipe: for object Litterfiles, we're going to read each element using read.csv function.
Litter <- LitterFiles %>%
  ldply(read.csv)
```
...
#Now let's select a few columns of interest using "select" to choose columns.
...
```{r}
# Notice that when you run this code, you won't have any summary printouts.
# This is because you're assigning a name to it (NTL.PeterPaul.summaries)
# BUT why can't we see the data grouped by lake and month? I thought this was what we 
# asked for?
